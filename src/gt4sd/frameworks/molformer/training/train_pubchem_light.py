...
class LightningModule(pl.LightningModule):
    def __init__(self, config, vocab):
        ...
        n_vocab, _ = len(vocab.keys()), config.n_embd
...
class MoleculeModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        loader = DataLoader(
            self.pubchem, collate_fn=self.text_encoder.process, **self.train_args
        )
...
def main():
    ...
    if config.num_nodes > 1:
        # logger.info("Using " + str(config.num_nodes) + " Nodes----------------------------------------------------------------------")
        ...
        logger.info(
            f"{os.environ['HOSTNAME']} MASTER_ADDR: {os.environ['MASTER_ADDR']}"
        )
        logger.info(
            f"{os.environ['HOSTNAME']} MASTER_PORT: {os.environ['MASTER_PORT']}"
        )
        logger.info(f"{os.environ['HOSTNAME']} NODE_RANK {os.environ['NODE_RANK']}")
        logger.info(
            f"{os.environ['HOSTNAME']} NCCL_SOCKET_IFNAME: {os.environ['NCCL_SOCKET_IFNAME']}"
        )
        logger.info(f"{os.environ['HOSTNAME']} NCCL_DEBUG: {os.environ['NCCL_DEBUG']}")
        logger.info(
            f"{os.environ['HOSTNAME']} NCCL_IB_CUDA_SUPPORT: {os.environ['NCCL_IB_CUDA_SUPPORT']}"
        )
        logger.info(f"Using {config.num_nodes} Nodes")
        logger.info(f"Using {torch.cuda.device_count()} GPUs")
    else:
        logger.info(f"Using {config.num_nodes} Node")
        logger.info(f"Using {torch.cuda.device_count()} GPUs")
